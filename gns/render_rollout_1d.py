...
def main(unused_argv):   
  ...
  for ax_i, (label, rollout_field) in enumerate(
    ...
    points = {
        particle_type: ax.plot([], [], "o", ms=6, color=color)[0]
  ...
  def update(step_i):
    ...
    for _, trajectory, points in plot_info:
      for particle_type, line in points.items():
        ...
        outputs.append(line)
    if step_i == 2: plt.savefig('rollouts/Concrete1D/' + FLAGS.output_path.split('/')[-1].split('.')[0] + f'_frame{step_i}.png')
    return outputs
  ...
  unused_animation.save(FLAGS.output_path, dpi=80, fps=5, writer='pillow')
