...
class Render():
    ...
    def __init__(self, input_dir, input_name):
        ...
        rollout_cases = [
            ["pred_trajs", "LS-DYNA"], ["pred_trajs", "GNN"]]
        strain_cases = [
            ["pred_strain", "LS-DYNA"], ["pred_strain", "GNN"]]
        ...
        for rollout_case in rollout_cases:
            trajectory[rollout_case[0]] = rollout_data[rollout_case[0]]
        strain = {}
        ...
        self.mask = rollout_data['particle_type'] != -1
        ...
        def animate(i):
            ...
            for j, datacase in enumerate(trajectory_datacases):
                if render_datacases[j] == "LS-DYNA":
                    color_map = self.color_map('pred_strain')
                else:
                    color_map = self.color_map('pred_strain')
